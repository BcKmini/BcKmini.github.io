    ...
    tech: [
      {
        ...
        desc: "AWS 리소스가 전부 콘솔로 수동 생성돼 있어 상태를 코드로 추적할 수 없었는데, terraform import로 기존 리소스를 하나도 건드리지 않고 그대로 state에 편입시켰습니다(plan 결과 0 add / 0 destroy). 이 과정에서 DLM 백업 대상이 태그 하나로 결정되고 보안그룹 description을 붙이면 delete+recreate가 발생한다는 걸 직접 겪고서야 알게 됐는데, 클라우드 리소스는 문서보다 provider 동작을 실제로 확인해야 안전하다는 걸 실감했습니다.",
        desc_en: "Every AWS resource had been created by hand through the console with nothing tracked as code, so I brought it all into Terraform via `terraform import` without touching a single resource (plan showed 0 add / 0 destroy). Along the way I learned, the hard way, that DLM picks backup targets by a single tag and that adding a security-group description triggers delete+recreate — provider behavior has to be verified firsthand, not assumed from docs.",
      },
      {
        name: "Prometheus+Grafana 모니터링 스택 구축",
        desc: "장애가 나도 kubectl로 파드를 하나씩 들여다보는 것 말고는 볼 방법이 없었습니다. 별도 monitoring 네임스페이스에 node-exporter·kube-state-metrics·Prometheus·Grafana를 리소스를 최소화해서 새로 올렸는데, 모니터링 스택 자체가 운영 워크로드를 침범하면 안 되니 노드 여유를 먼저 실측하고 나서 설계해야 한다는 걸 배웠습니다.",
        desc_en: "With no observability, the only way to check anything was kubectl, pod by pod. I stood up node-exporter, kube-state-metrics, Prometheus, and Grafana in an isolated namespace, sized to a minimum footprint — a monitoring stack has to earn its own resource budget without eating into production's, so I measured headroom first and designed around it.",
      },
      {
        name: "Promtail 장애 진단 → Grafana Alloy 교체",
        desc: "Loki+Promtail로 로그 수집을 붙였는데 파드 디스커버리가 조용히 멈추는 문제를 만났습니다. RBAC 권한, API 직접 접근, 로그 파일 글롭 패턴, 버전 차이까지 가능한 원인을 하나씩 배제해 결국 Promtail 자체의 버그로 결론짓고, 공식 후속인 Grafana Alloy로 교체했습니다. 흔한 원인(RBAC, 볼륨)이 전부 정상으로 확인되면 도구 자체를 의심해야 한다는 걸 이때 배웠습니다.",
        desc_en: "After wiring up Loki+Promtail for log collection, pod discovery silently died. I ruled out RBAC, direct API access, the log-glob pattern, and two versions one at a time, concluded it was Promtail's own discovery bug, and switched to its official successor, Grafana Alloy. Once every usual suspect — RBAC, volumes — checks out clean, the tool itself becomes the suspect.",
      },
      {
        name: "GitHub Actions 상태체크 + rollout 오탐 진단",
        desc: "ai 서버 배포가 반복 실패했는데 코드 문제인지 인프라 문제인지 알 수 없었습니다. 10분마다 HTTP 헬스체크와 실제 kubectl 파드 상태를 함께 확인하는 워크플로우를 만들어 이벤트 로그를 직접 들여다봤더니, BGE-M3+reranker 모델이 반영되며 이미지가 5GB로 커져 pull에만 7분 넘게 걸리는 게 진짜 원인이었습니다. 타임아웃을 늘려 해결하면서, 배포 실패가 항상 코드 탓은 아니고 타임아웃 값 자체가 잘못된 기준일 수 있다는 걸 배웠습니다.",
        desc_en: "The ai server's deploys kept failing and it wasn't clear whether the cause was code or infra. I built a workflow that checks real kubectl pod status alongside an HTTP health check every 10 minutes, and the event log showed the image had grown to 5GB once the BGE-M3+reranker model got baked in, taking over 7 minutes just to pull. Raising the timeout fixed it — a deploy failure isn't always a code bug, sometimes the timeout itself is the wrong assumption.",
      },
      {
        name: "k3s 실운영 장애 진단·수정",
        desc: "배포는 됐지만 livenessProbe 부재, PVC fsGroup 누락, OOMKilled 같은 문제들이 조용히 서비스를 망가뜨리고 있었습니다. 로그와 kubectl 상태를 하나씩 대조해 원인을 찾아 고치고, 모든 워크로드에 resource requests/limits를 설정했습니다. '배포됐다'와 '제대로 동작한다'는 서로 다른 말이고, 직접 상태를 찍어봐야만 그 차이가 보인다는 걸 이 과정에서 계속 확인했습니다.",
        desc_en: "The app was deployed, but a missing livenessProbe, a PVC without fsGroup, and OOMKills were quietly breaking things underneath. I diagnosed each by cross-checking logs against kubectl status and set requests/limits on every workload — \"deployed\" and \"actually working\" are different claims, and the gap only shows up once you check state directly.",
      },
      {
        name: "백업 복구 리허설 + 노드 용량 재해석",
        desc: "백업은 매일 쌓이고 있었지만 실제로 복구되는지 검증된 적이 없었고, 노드도 CPU가 다 찼다고 오판한 상태였습니다. 별도 인스턴스로 스냅샷 복구를 직접 리허설해 데이터 무결성을 확인했고, kubectl top으로 실사용률이 3%에 불과하다는 걸 실측해 근거 없는 리사이즈를 막았습니다. '쌓이고 있다'가 '복구된다'를 보장하지 않고, limits 합계만으로 여유를 판단하면 안 된다는 걸 이 두 가지로 다시 확인했습니다.",
        desc_en: "Snapshots were piling up daily with no verification they'd actually restore, and I'd misjudged the node as CPU-maxed. I ran a real restore drill on a separate instance to confirm data integrity, and measured actual usage at just 3% with kubectl top — heading off an unnecessary resize. Both cases came down to the same lesson: accumulating isn't the same as restorable, and summing `limits` alone doesn't tell you real headroom.",
      },
      {
        name: "라이브 QA로 찾은 Server 버그 2건",
        desc: "근로자 보안 링크 발급이 특정 업무카드에서 계속 실패해 원인을 추적하니, 데모 시드가 실제 로직으로는 나올 수 없는 상태를 만들고 있었습니다. 이걸 고치고 로그아웃 상태로 재검증하다 두 번째 버그도 찾았는데, 근로자가 로그인 없이 쓰는 공개 API 3개가 접두사 없이 매핑돼 실제 배포 환경에서는 접근 자체가 안 되고 있었습니다. 통합 테스트가 다 통과해도, 실제로 로그아웃 상태에서 눌러봐야만 보이는 버그가 있다는 걸 이때 배웠습니다.",
        desc_en: "Issuing a worker security link kept failing on certain task cards, and tracing it back showed the demo seed was faking a state real application logic could never produce. Fixing that and re-verifying while logged out surfaced a second bug — three public worker APIs mapped without a prefix, unreachable in the real deployment. Passing integration tests don't guarantee a working flow; some bugs only surface by actually clicking through it logged out.",
      },
      {
        name: "mock → 실데이터 전환 + 근로자 보안 링크",
        desc: "업무함 화면이 mock 데이터라 실제로 쓸 수 없었고, 근로자 메뉴가 아예 없어 목록에 들어갈 방법도 없었습니다. Server API로 전면 전환하고 근로자 보안 링크 발급·재발급과 SOLAPI 실제 SMS 발송까지 연결했는데, 기능을 다 만들어도 진입 경로 하나가 빠지면 아무도 그 기능에 도달하지 못한다는 걸 이 사이드바 버그로 실감했습니다.",
        desc_en: "The task screen ran on mock data, and the worker menu was missing entirely — there was no way in. I switched everything to the real Server API and wired up worker security-link issue/reissue plus real SMS delivery via SOLAPI; that missing sidebar entry was a reminder that a feature can be fully built and still be unreachable if one entry point is gone.",
      },
    ],
