export const projects = [
    ...
    tech: [
      {
        ...
        desc: "PDF OCR·Whisper STT 같은 무거운 AI 연산이 API 프로세스를 블로킹해 타임아웃이 나던 문제를, 업로드 응답은 즉시 반환하고 추론은 백그라운드 워커로 넘기는 비동기 구조로 해결. 무거운 연산이 도는 동안에도 다른 요청이 막히지 않게 됐습니다.",
      },
      {
      ...
      },
      {
        ...
        desc: "웹과 데스크톱을 따로 만들면 인증 로직을 두 벌로 구현하고 두 벌로 고쳐야 하는 문제가 있어, 같은 빌드 산출물을 공유하는 크로스 플랫폼 구조로 설계. 소셜 로그인 3사(Google/Naver/Kakao)와 JWT 인증 인터셉터를 공통 모듈로 묶어 웹·데스크톱 어디서 로그인해도 같은 코드가 처리하게 했습니다.",
      },
    ],
  ...
  },
  {
    ...
    tech: [
      ...
      },
      {
        ...
        desc: "같은 민원 질의가 반복되는 특성을 이용해, 동일 질문은 LLM을 다시 호출하지 않고 캐시에서 즉시 응답하도록 엔드포인트 단에 캐시 레이어를 추가 — 반복 질의의 LLM API 호출 자체를 없애 동시 접속 상황에서도 응답이 밀리지 않게 했습니다.",
      },
      {
        name: "Whisper STT + edge-tts",
        desc: "타이핑이 익숙하지 않은 고령층은 기존 텍스트 기반 챗봇을 쓰기 어렵다는 문제가 있어, 음성으로 묻고 음성으로 답을 듣는 STT·TTS 파이프라인을 연동해 타이핑 없이도 전체 기능을 이용할 수 있게 했습니다.",
      },
    ],
  ...
  },
  {
    ...
    tech: [
      ...
      },
      {
        ...
        desc: "YOLO가 놓치거나 잘못 잡은 구간을 고칠 방법이 없으면 결과를 그대로 못 쓰는 문제가 있어, 연구원이 드래그로 직접 보정할 수 있는 편집 타임라인을 만들고, 확정 구간을 MP4 클립·CSV·JSON으로 일괄 내보내 외부 통계 분석과 바로 연계되게 했습니다.",
      },
    ],
  ...
  },
  {
    ...
    tech: [
      ...
      {
        name: "지수 백오프 + 메모리 캐싱",
        desc: "외부 서비스 응답 지연이 메인 서버 타임아웃으로 번지는 문제를 재시도 로직과 동일 좌표·시간대 캐싱으로 차단 — 운영 기간 중 외부 장애가 서비스 전체로 전파된 사례는 0건입니다.",
      },
      {
      ...
      {
        name: "도메인 분리 아키텍처",
        desc: "한 파일에 몰려 있던 프로토타입에서는 API 하나만 바뀌어도 파일 전체를 훑어야 했는데, FastAPI로 마이그레이션하며 Router·Model·Schema 계층으로 재구조화한 뒤로는 변경 범위를 해당 계층 파일로 좁혀 확인할 수 있게 됐습니다.",
      },
    ],
