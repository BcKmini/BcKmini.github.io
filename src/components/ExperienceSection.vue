<script setup>
import { experience } from "../data/experience";
import { isOngoing } from "../composables/useOngoing";
import { vReveal } from "../composables/useReveal";
import { useLocale } from "../composables/useLocale";

const { t } = useLocale();
</script>

<template>
  <div class="section" v-reveal data-snap-section data-key="experience" :data-label="t('경험', 'Experience')">
    <h2 class="sec"><span>03.</span> {{ t("경험", "Experience") }}</h2>
    <p class="sec-sub">{{ t('구체적인 역할과 산출물이 있는 경험입니다. 짧은 교육 이수·활동 이력은 아래 "자격 · 활동"에 따로 정리했습니다.', 'Experience with a concrete role and deliverables. Shorter courses/activities are listed separately below under "Certificates · Activities."') }}</p>

    <div class="term-window">
      <div class="term-bar">
        <span class="term-dot term-dot-red"></span>
        <span class="term-dot term-dot-amber"></span>
        <span class="term-dot term-dot-green"></span>
        <span class="term-bar-title">git log --oneline --graph experience/</span>
      </div>
      <ul class="git-log">
        <li v-for="item in experience" :key="item.title + item.date" class="git-row" :class="{ 'git-row-live': isOngoing(item) }">
          <div class="git-graph">
            <span class="git-dot" :class="{ live: isOngoing(item) }"></span>
            <span class="git-line"></span>
          </div>
          <div class="git-content">
            <p class="git-meta">
              <span class="git-date">{{ item.date }}</span>
              <span v-if="isOngoing(item)" class="live-pill"><span class="live-pill-dot"></span>{{ t("진행중", "Ongoing") }}</span>
            </p>
            <h3>
              <a v-if="item.link" :href="item.link" target="_blank" rel="noopener">{{ t(item.title, item.title_en) }} ↗</a>
              <template v-else>{{ t(item.title, item.title_en) }}</template>
              <em v-if="item.type" class="tag tag-accent">{{ t(item.type, item.type_en) }}</em>
            </h3>
            <p v-if="item.descHtml" v-html="t(item.descHtml, item.descHtml_en)"></p>
            <p v-else>{{ t(item.desc, item.desc_en) }}</p>
          </div>
        </li>
      </ul>
    </div>
  </div>
</template>
